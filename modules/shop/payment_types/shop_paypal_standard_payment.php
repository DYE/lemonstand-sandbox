<?
	...
	class Shop_PayPal_Standard_Payment extends Shop_PaymentType
	{
		...
		public function build_config_ui($host_obj, $context = null)
		{
			...
			$host_obj->add_field('cancel_page', 'Cancel Page', 'left')->tab('Configuration')->renderAs(frm_dropdown)->comment('Page to which the customer’s browser is redirected if payment is cancelled.', 'above');
		...
		public function process_paypal_ipn($params)
		{
			try
			{
				...
				$fields = $_POST;
				if($order->payment_method->test_mode)
				...
				{
					if (post('mc_gross') != strval($this->get_paypal_total($order)))
			...
			try
			{
				...
				if (!$order->payment_processed(false))
				{
					...
					if (strpos($response, 'SUCCESS') !== false)
					{
						...
						if ($matches[1] != strval($this->get_paypal_total($order)))
		...
		*/
		private function get_paypal_total($order)
		{
			$order_total = 0;
