<?
	 ...
	 * {@link Shop_Cart} class. Please read the {@link http://lemonstandapp.com/docs/cart_page/ Creating the Cart page} 
	...
	class Shop_CartItem
	{
		...
		public function discount($total_discount = true)
		{
			...
			if (!$this->price_is_overridden($effective_quantity))
			{
				$om_record = $this->get_om_record();
				if (!$om_record)
					return round($this->product->get_sale_reduction($effective_quantity), 2);
				else
		...
		public function total_price_no_tax($apply_cart_level_discount = true, $quantity = null)
		{
			...
			return ($this->single_price_no_tax(true, $quantity) - $catalog_level_discount - $cart_level_discount)*$quantity;
		}
		 ...
		 * @param boolean $force_tax Determines whether taxes should be added to the result, overriding {@link http://lemonstandapp.com/docs/configuring_lemonstand_for_tax_inclusive_environments/ Display catalog/cart prices including tax} option status.
		 ...
		 * {@link http://lemonstandapp.com/docs/allowing_customers_to_provide_order_item_specific_information/ this article}. 
		 * Example: 
		 * <pre>
		 * <textarea 
		...
		/**
		 * Returns a list of files uploaded by the customer on the {@link http://lemonstandapp.com/docs/supporting_file_uploads_on_the_product_page/ Product Details page}. 
		 * Use this method to display files assigned with items on the {@link http://lemonstandapp.com/docs/cart_page Cart page}. 
