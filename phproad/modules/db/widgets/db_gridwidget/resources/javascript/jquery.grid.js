...
(function( $, undefined ) {
	jQuery.widget( "ui.grid", {
		...
		appendRow: function(currentCell, ev) {
			...
			var cell = currentCell !== undefined ? currentCell : (ev.target !== undefined ? jQuery(ev.target).closest('table.ui-grid-body tr td') : null);
		...
		_createRowMenu: function(row, cell) {
			var 
				contentContainer = cell.find('.cell-content-container');
				menu = jQuery('<span class="ui-grid-menu"></span>'),
				self = this,
				deleteLink = jQuery('<a class="menu-delete">Delete</a>').attr('tabindex', 0).bind('click', function(ev){
