	...
	class Shop_Orders extends Backend_Controller
	{
		...
		public function __construct()
		{
			...
			if (post('find_product_mode'))
			{
				...
				$this->list_columns = array('name', 'sku');
				...
				$this->list_search_fields = array('@name', '@sku');
			...
			try
			{
				...
				foreach ($order_ids as $order_id)
				{
					...
					try
					{
						...
						Shop_OrderStatusLog::create_record($data['status_id'], $order, $data['comment'], $data['send_notifications']);
						$orders_processed++;
					}
					catch (Exception $ex)
					{
		...
		{
			return Shop_Product::create()->where('grouped is null');
		}
		...
		protected function getAvailablePaymentMethods($order)
		{
			...
			return Shop_PaymentMethod::list_applicable($order->billing_country_id, $order->total, true)->as_array();
		}
		...
		{
			return (strlen($id) && $id != 'invoice') ? $this->formFindModelObject($id) : $this->formCreateModelObject();
		}
