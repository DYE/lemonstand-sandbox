...
var FancyUpload2 = new Class({
	...
	onAllSelect: function(files, current, overall) {
		...
		if (this.status) this.status.removeClass('status-browsing');
	...
	onCancel: function() {
		...
		if (this.status) this.status.removeClass('file-browsing');
	},
	...
	onAllComplete: function(current) {
		...
		if (this.status) this.status.removeClass('file-uploading');
	},
	...
	browse: function(fileList) {
		...
		if (ret !== true){
			...
			if (this.status) this.status.addClass('file-browsing');
		}
	},
	...
	upload: function(options) {
		...
		if (ret !== true) {
			...
			if (this.status) this.status.addClass('file-uploading');
			if (this.overallProgress !== undefined && this.overallProgress)
	...
	finishFile: function(file) {
		...
		{
			file = this.getFile(file);
			if ($(file.element)) file.element.removeClass('file-uploading');
			file.finished = true;
			return file;
		}
		catch(err)
