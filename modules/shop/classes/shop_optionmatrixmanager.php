<?
	...
	/**
	 * Provides methods for creating and updating {@link http://lemonstandapp.com/docs/understanding_option_matrix/ Option Matrix} records.
	 * @documentable
	...
	class Shop_OptionMatrixManager
	{
		...
		public function get_record($product, $options)
		{
				...
				$filters[] = sprintf(
					'exists(
						...
						where 
							...
							and shop_custom_attributes.name=\'%s\' 
			...
			$result = Db_DbHelper::object('select * from shop_option_matrix_records where product_id=:product_id and '.implode(' and ', $filters), array('product_id'=>$product));
			if (!$result)
				return null;
		...
		public function add_or_update($product, $options, $data, $skip_existing = false)
		{
			...
			if (!$existing_record)
			{
				...
				foreach ($options as $option_name=>$option_value)
				{
					Db_DbHelper::query('insert into shop_option_matrix_options
						(matrix_record_id, option_id, option_value) values 
						(:record_id, :option_id, :option_value)', array(
							'record_id'=>$record_id, 
