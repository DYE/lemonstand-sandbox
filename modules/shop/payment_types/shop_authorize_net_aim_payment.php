<?
	...
	class Shop_Authorize_Net_Aim_Payment extends Shop_PaymentType
	{
		...
		public function process_payment_form($data, $host_obj, $order, $back_end = false)
		{
			...
			$customer_profile = isset($data['create_customer_profile']) ? $this->update_customer_profile($host_obj, $order->customer, $data) : null;
			...
			try
			{
				...
				$fields['x_amount'] = $currency_converter->convert($order->total, $currency->code, 'USD');
		...
		public function pay_from_profile($host_obj, $order, $back_end = false, $redirect = true)
		{
			...
			$transaction->amount = $currency_converter->convert($order->total, $currency->code, 'USD');
			$transaction->tax->amount = $currency_converter->convert($order->goods_tax + $order->shipping_tax, $currency->code, 'USD');
			...
			foreach ($order->items as $item)
			{
				...
				$lineItem->unitPrice = $currency_converter->convert($item->unit_total_price, $currency->code, 'USD');
			...
			if ((float)$order->shipping_quote)
			{
				...
				$lineItem->quantity = 1;
				$lineItem->unitPrice = $currency_converter->convert($order->shipping_quote, $currency->code, 'USD');
